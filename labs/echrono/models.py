...
class Glider(models.Model):
    ...
    time_in_air = models.CharField("time_in_air", blank=False, default="-",max_length=15)
...
class Chronometer(models.Model):
    ...
    time_of_start = models.DateTimeField("time_of_start")
    glider_landing_time = models.DateTimeField("glider_landing_time")
    airplane_landing_time = models.DateTimeField("airplane_landing_time")
    glider_tia = models.CharField("glider_tia", blank=False, default="-",max_length=15)
    airplane_tia = models.CharField("glider_tia", blank=False, default="-",max_length=15)
    start_type = models.CharField("start_type", max_length=1,blank=False)
    active = models.BooleanField("active",default=False)

    winch_operator_id = models.ForeignKey(User,on_delete=models.DO_NOTHING,blank=True,default='',related_name='winch')
    tow_pilot_id = models.ForeignKey(User,on_delete=models.DO_NOTHING,blank=True,default='',related_name='tow')
    instructor_id = models.ForeignKey(User,on_delete=models.DO_NOTHING,blank=True,default='',related_name='instruct')
